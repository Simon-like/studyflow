...
export function GroupCard({ group, onJoin, onLeave }: GroupCardProps) {
  const colorClass = CATEGORY_COLORS[group.category] || "bg-sage";
  const creatorName =
    group.creator?.nickname || group.creator?.username || "创建者";

  return (
    <Card hover className="h-full">
      <div
        className={`w-12 h-12 ${colorClass} rounded-xl flex items-center justify-center mb-4 shadow-soft`}
      >
        <svg
      ...
      <p className="text-stone text-xs mb-1">
        {group.memberCount} 人正在学习
        {group.category && ` · ${group.category}`}
      </p>
      {group.description && (
        <p className="text-stone text-xs mb-3 line-clamp-2">
          {group.description}
        </p>
      )}
...
export type { GroupCardProps, StudyGroup } from "./types";
